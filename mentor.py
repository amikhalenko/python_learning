...
def compare_lecturer():
    for keys,values in mid_lecturer.items():
        list_values_lecturers.append(values)

        for i in range(1, len(list_values_lecturers)):
            a, b = list_values_lecturers[i - 1], list_values_lecturers[i]
            if a < b:
                print('{} < {}'.format(a, b))
            elif a > b:
                print('{} > {}'.format(a, b))
